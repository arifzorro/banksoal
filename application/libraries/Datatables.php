...
class Datatables
{
    ...
    private function get_total_results($filtering = FALSE)
    {
        ...
        if (count($this->having) > 0) {
            ...
            $this->ci->db->select($this->real_columns);
        }

        if (count($this->or_having) > 0) {
            foreach($this->or_having as $val) {
                $this->ci->db->or_having($val[0], $val[1], $val[2]);
            }
            $this->ci->db->select($this->real_columns);
        }
